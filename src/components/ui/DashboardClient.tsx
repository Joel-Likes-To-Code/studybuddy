...
export default function DashboardClient({
  ...
  async function saveToServer(input: NewCardInput) {
    // Example POST to your API; adjust to your schema (prompt/answer/etc.)
    ...
    const saved = (await res.json()) as CardDTO;
  ...
  return (
    <main className="max-w-5xl mx-auto p-6 space-y-6">
      ...
      <p className="text-lg">Welcome back, <span className="font-medium">{userName}</span>!</p>
      ...
      <section className="space-y-3">
        ...
        {cards.length === 0 ? (
          ...
          <ul className="grid gap-3">
            {cards.map((c) => (
              <li key={c.id} className="rounded-md border border-[var(--border)] p-3 bg-[var(--card)]">
                <div className="text-sm font-medium">{c.prompt}</div>
                <div className="text-sm opacity-80 mt-1">{c.answer}</div>
                {c.tags?.length ? (
                  <div className="mt-2 flex flex-wrap gap-1">
                    {c.tags.map((t) => (
                      <span key={t} className="px-2 py-0.5 text-xs rounded-full bg-[var(--muted)]">
                        {t}
                      </span>
                    ))}
                  </div>
          ...
          <div>
            ...
            <div className="rounded-md border border-[var(--border)] p-2 bg-[var(--card)]">
              <div className="flex flex-wrap gap-2">
                {tags.map((t) => (
                  <span key={t} className="inline-flex items-center gap-1 rounded-full bg-[var(--muted)] text-[var(--foreground)]/80 px-2 py-0.5 text-xs">
                    {t}
                    <button
                      type="button"
          ...
          <div className="pt-2 flex items-center gap-2">
            <button
              ...
              className={`px-4 py-2 rounded-md text-[var(--primary-foreground)] text-sm transition-opacity ${
                canSave && !saving ? "bg-[var(--primary)] hover:opacity-90" : "bg-[var(--primary)] opacity-50 cursor-not-allowed"
              }`}
            >
...
function FlipCard({
  ...
  isFlipped: boolean;
  onToggle: () => void;
  children: React.ReactNode; // expects [front, back]
}) {
  ...
  return (
    <div className="relative [perspective:1000px] max-w-xl w-full">
      <div
        className={`relative h[300px] h-[300px] w-full rounded-lg border border-[var(--border)] bg-[var(--card)] text-[var(--card-foreground)] transition-transform duration-500 [transform-style:preserve-3d] ${isFlipped ? "[transform:rotateY(180deg)]" : ""}`}
      >
        <button type="button" aria-label="Flip card" onClick={onToggle} className="absolute inset-y-0 right-0 w-8 rounded-r-lg hover:bg-[var(--muted)]/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ring)] z-10" />
        <button type="button" aria-label="Flip card" onClick={onToggle} className="absolute inset-y-0 left-0 w-8 rounded-l-lg hover:bg-[var(--muted)]/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ring)] z-10" />
